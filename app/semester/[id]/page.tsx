...
const coursesBySemester: Record<number, { name: string; faculty: string; email: string; code: string }[]> = {
  1: [
  ...
  4: [
    ...
    { code: "CS403", name: "Design & Analysis of Algorithms", faculty: "Dr. Ramya S", email: "ramya@bmsce.ac.in" },
...
const colors = [
  "bg-orange-500", "bg-indigo-600", "bg-emerald-500",
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-indigo-50">
      <nav className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center gap-4">
          <button onClick={() => router.push("/")} className="text-gray-400 hover:text-orange-500 font-bold text-sm">
            ← Back
          </button>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-gray-800">Semester {semId} Courses</h1>
          <p className="text-gray-500 mt-1">Select a course to view notes and materials</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5">
          {courses.map((course, i) => (
            <button
              key={course.code}
              onClick={() => router.push(`/semester/${semId}/course/${course.code}`)}
              className="bg-white border-2 border-gray-100 hover:border-orange-300 rounded-2xl p-6 text-left shadow-sm hover:shadow-lg transition-all group"
            >
              <div className={`inline-flex items-center justify-center w-12 h-12 ${colors[i % colors.length]} rounded-xl mb-4`}>
                <span className="text-white font-black text-sm">{course.code.slice(0,2)}</span>
              </div>
              <h3 className="font-black text-gray-800 text-lg group-hover:text-orange-600 transition-colors">
                {course.name}
              </h3>
